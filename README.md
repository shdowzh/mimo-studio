...
## V1.5.0 新特性 — 任意文件附件 + 模型能力提示

继 v1.4.0 之后继续打磨附件体验，并解决"模型不支持图片/二进制"导致的报错困惑：

- **📎 任意文件附加**：v1.4.0 仅支持图片和文本/代码，本版本放开**任意二进制文件**（pdf / docx / xlsx / zip…）。binary 文件以**绝对路径**形式拼进消息，Agent 用 Read / Bash 等工具按需读取，不会浪费上下文
- **🎨 视觉区分**：binary 附件 chip 用**虚线边框 + 扩展名徽章**（PDF / XLSX / DOCX）和"内联附件"明确区分；hover 提示"将以路径形式发送，AI 会按需读取"
- **⚠️ 模型能力事前提示**：附加图片时，若当前模型明确不支持视觉（DeepSeek-V3、Qwen Turbo、Moonshot v1 等），chip 显示黄色 ⚠️ 角标 + 警告描边，引导用户切换到 GPT-4o / Claude / Gemini Pro 等多模态模型
- **🇨🇳 错误消息中文化**：Provider 报错（vision-unsupported / 401 / 429 / 上下文超限 / 网络）自动转译成中文友好提示 + 建议下一步操作，不再扔英文 stack trace 给用户
- **🚧 离线模式拦截**：MiMo Serve 离线（fallback 模式）时禁止添加 binary 路径附件，明确提示"AI 无法读取本地路径，请启动 MiMo Serve 或仅附加图片"
- **📦 容量上限调整**：图片仍 10MB，文本仍 2MB，新增 binary 上限 100MB（仅传路径不读内容，主要防误选 ISO 镜像）
...
| 平台 | 下载 | 大小 | 说明 |
|------|------|------|------|
| 🪟 **Windows** (x64) 安装版 | [⬇ 下载 exe](https://gitee.com/shdowzh/mimo-studio/releases/download/v1.5.0/MiMo-Studio-Setup-1.5.0-win-x64.exe) | ~86 MB | NSIS 安装包，支持自定义安装目录 |
| 🐧 **Linux** (x64) tar.xz | [⬇ 下载 tar.xz](https://gitee.com/shdowzh/mimo-studio/releases/download/v1.5.0/MiMo-Studio-1.5.0-linux-x64.tar.xz) | ~77 MB | 解压即用：`tar xf` 后运行 `./mimo-studio` |
| 🍎 **macOS** (Apple Silicon M1-M4) | [⬇ 下载 tar.xz](https://gitee.com/shdowzh/mimo-studio/releases/download/v1.5.0/MiMo-Studio-1.5.0-mac-arm64.tar.xz) | ~70 MB | 解压即用：`tar xf` 后运行 `MiMo Studio.app` |
| 🍎 **macOS** (Intel) | [⬇ 下载 tar.xz](https://gitee.com/shdowzh/mimo-studio/releases/download/v1.5.0/MiMo-Studio-1.5.0-mac-x64.tar.xz) | ~77 MB | 解压即用：`tar xf` 后运行 `MiMo Studio.app` |
