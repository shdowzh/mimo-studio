...
<p align="center">
  ...
  <a href="https://gitee.com/shdowzh/mimo-studio/releases"><img src="https://img.shields.io/badge/release-v1.1.5-green.svg" /></a>
...
| 平台 | 文件 | 大小 | 说明 |
|------|------|------|------|
| 🪟 **Windows** (x64) 安装版 | `MiMo-Studio-Setup-1.1.0-win-x64.exe` | ~86 MB | NSIS 安装包，可选安装目录 |
| 🪟 **Windows** (x64) 免安装版 | `MiMo-Studio-1.1.0-win-x64-Portable.tar.xz` | ~80 MB | 解压即用：`tar xf` 后运行 `MiMo Studio.exe` |
| 🐧 **Linux** (x64) tar.xz | `MiMo-Studio-1.1.0-linux-x64.tar.xz` | ~77 MB | 解压即用：`tar xf` 后运行 `./mimo-studio` |
| 🐧 **Linux** (x64) deb | `MiMo-Studio-1.1.0-linux-amd64.deb` | ~90 MB | Debian/Ubuntu：`dpkg -i` 安装 |
| 🍎 **macOS** (Apple Silicon) tar.xz | `MiMo-Studio-1.1.0-mac-arm64.tar.xz` | ~70 MB | 解压即用：`tar xf` 后运行 `MiMo Studio.app` |
| 🍎 **macOS** (Intel) tar.xz | `MiMo-Studio-1.1.0-mac-x64.tar.xz` | ~77 MB | 解压即用：`tar xf` 后运行 `MiMo Studio.app` |

> **macOS DMG 安装包**（~110 MB）和 **Linux AppImage**（~116 MB）在 [GitHub Releases](https://github.com/shdowzh/mimo-studio/releases) 下载。码云单文件限制 100MB，tar.xz/zip 格式通过压缩适配。<br>
