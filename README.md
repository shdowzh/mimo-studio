...
| 🍎 **macOS** | 通过 GitHub Actions 构建 | — | 见下方说明 |

> **macOS 用户**：由于 macOS 应用只能在 macOS 系统上编译，请通过以下方式获取：
> 1. **从源码构建**：`git clone git@gitee.com:shdowzh/mimo-studio.git && cd mimo-studio && npm install && npm run electron:build:mac`
> 2. **GitHub Actions**：推送 tag 后自动构建全平台包（`.github/workflows/release.yml`）
...
1. 启动后进入引导程序 → 点击"开始使用"
2. 检测 MiMo CLI：推荐安装以获得完整 Agent 能力；也可跳过
3. 如跳过安装，空状态页会引导你去设置页安装 CLI 或配置外部 API Key
