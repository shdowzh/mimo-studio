...
### V8 → V9 UI 重设计（2026-06-20）

**背景：** V8 功能稳定但视觉体验粗糙——4 种不同字号任意值散落各处、色彩语义不统一（多处硬编码 `#fff`/`bg-zinc-800`）、ChatView 状态横幅堆叠 4 条、消息气泡左右跳动、Tool 卡片 2-3 行堆叠、各 View 顶栏散落拖拽占位 div、Settings/Skills 等子页信息密度低且无标准空态。分 4 个 Phase 系统化改造全站 UI，建立 design token 体系。

**Phase 1 — 视觉基建：**

| 模块 | 旧实现 | 新实现 |
|------|--------|--------|
| 色彩系统 | 5 套主题各写各的，无统一语义 token | 引入 `mc-*` 语义 token：`mc-bg` / `mc-surface` / `mc-elevated` / `mc-hover` / `mc-border` / `mc-border-subtle` / `mc-brand` / `mc-brand-hover` / `mc-brand-soft` / `mc-accent` |
| 品牌色 | 无强调色，整体灰蓝冷淡 | dark/light 主题使用 indigo 品牌色；Nord/Catppuccin/One Dark 沿用各自原生强调色 |
| 字号 | `text-[9px]` / `text-[10px]` / `text-[11px]` 99 处任意值 | 收敛到 `text-2xs (11px) / text-xs (12px) / text-sm (13px) / text-base (14px) / text-lg (16px)` |
| 按钮 | `variant: primary/secondary/ghost/danger` | 新增 `variant="brand"` + focus-ring |
| 输入框 | `variant: underline` | 新增 `variant="box"` 适配卡片内表单 |
| 模态框 | fade-in 动画 | slide-up 动画 + 标题分隔线改 subtle |
| 滚动条 | 4px | 6px，视觉更明显 |
| 状态横幅 | ChatView 内 4 条堆叠（lastError/initError/initializing/offline） | `<ChatStatusBar />` 优先级整合，用户一次只看一条 |

**Phase 2 — 主导航与会话列表：**

| 模块 | 旧实现 | 新实现 |
|------|--------|--------|
| Sidebar | 固定 52px，仅图标 + hover tooltip | 默认 52px，hover 展开 220px 显示文字 + 分组标题 |
| Sidebar 分组 | NavItem 平铺 5 个 | 三组：工作区（聊天/终端）、知识（记忆/技能/MCP）、系统（设置） |
| Sidebar 底栏 | 设置按钮 | 上方加 ServerStatusBadge（StatusDot + Agent 在线/离线状态，点击跳 Provider 配置） |
| ConversationList | 按 updated 倒序平铺 | 搜索框（Ctrl+K 唤起）+ 时间分组（今天/昨天/本周/更早） |
| ConversationList右键 | 无 | ContextMenu：重命名/置顶/复制ID/删除 |
| 置顶会话 | 不支持 | 前端 SQLite 存 `pinned-sessions` 数组，置顶项排最上 |
| 各 View 顶栏 | 散落的 `<div className="h-[36px] drag" />` + 各行内 header | 统一用 `<TitleBar />` 组件（36px drag + title + actions + subBar） |

**Phase 3 — 聊天主区重设计：**

| 模块 | 旧实现 | 新实现 |
|------|--------|--------|
| 消息布局 | 用户右对齐、Assistant 左对齐，75% 最大宽度 | 统一左对齐，28px 头像列（User 头像/ Bot 品牌色头像）|
| Assistant 消息 | bg-surface bubble | `border-l-2 border-mc-brand/30 pl-4` 左侧品牌色细线标识 |
| StepBlock | 默认全部展开 | 正在执行的展开，已完成的折叠成单行摘要（`✓ N 个工具 · Xs · N tokens`）|
| ToolCallCard | 2-3 行堆叠 | 折叠态严格单行：图标 + 工具名 · 摘要 · StatusDot/Spinner 状态 · 耗时；点击展开详情 |
| ReasoningBlock | "查看思考"按钮 | 流式中「Spinner 思考中」，结束后默认折叠成「💭 思考过程 · N 字」斜体灰字 |
| MessageInput | 裸 textarea + 发送按钮 | 左侧 + 菜单（附加文件/技能 placeholder）、右下角 @模型 chip、focus brand ring、占位符暗示快捷键 |
| EmptyState | 4 条硬编码 QUICK_PROMPTS | 优先最近 5 条用户 prompt 历史（SQLite `recent-prompts`）+ 底栏安装 CLI/配置 Key 卡片保持 |
| PermissionDialog | 右下角 fixed，3 个 inline 按钮 | 品牌色描边 + 权限分类图标 + `<Button variant="brand/secondary/danger">` 组件 |

**Phase 4 — 子页面打磨：**

| 模块 | 旧实现 | 新实现 |
|------|--------|--------|
| SettingsView | 顶部 tab，Provider 卡片纵向单列、密度过高 | 左侧 tab 列 + 右侧内容区两栏；Provider 卡片 2 列网格 + 折叠态（默认显示 logo+名+状态+配置按钮）|
| SkillsView | 单一列表，特色卡片空 URL | 三栏布局（分类 + 列表 + 详情面板），列表改紧凑行而非网格 |
| MemoryView | 单一 textarea，无大纲导航、无保存反馈 | 左侧 Markdown 大纲解析 + 右侧编辑器 + 底部状态栏（字数·最后保存·Ctrl+S）|
| McpView | 列表 + 普通 modal | StatusDot 状态语义化 + EmptyHint 空态 + 品牌色按钮 |
| TerminalView | 仅 36px drag，字号写死 13px | TitleBar + subBar 工具条（PTY/本地标签 + 字号选择 + 清屏按钮）+ EmptyHint 空态 |
| 全项目 | `text-[9/10/11px]` 99 处 + 硬编码颜色分散 | 全部替换为 design token 类名 |

**新建文件：**
- `src/components/ui/TitleBar.tsx` — 统一视图顶栏组件（36px drag + title + actions + subBar）
- `src/components/ui/Spinner.tsx` — brand/muted 双 tone 加载转圈
- `src/components/ui/StatusDot.tsx` — success/warning/error/brand/muted 五 tone + pulse
- `src/components/ui/EmptyHint.tsx` — 通用空态布局（icon/title/description/action）
- `src/components/ui/ContextMenu.tsx` — 右键菜单组件
- `src/components/Sidebar/ServerStatusBadge.tsx` — Sidebar 底部服务状态徽标
- `src/views/ChatView/ChatStatusBar.tsx` — 优先级整合的聊天状态横幅
- `src/lib/recentPrompts.ts` — 最近 prompt 历史（SQLite 持久化）
- `UI-REDESIGN-PLAN.md` — 完整 UI 重设计计划与验收文档

**修改文件：**
- `src/styles/globals.css` — 5 套主题新增 `--brand` / `--brand-hover` / `--brand-soft`；调暗 dark 主题层级；滚动条 4px→6px
- `tailwind.config.js` — 新增 `mc-brand` / `mc-brand-hover` / `mc-brand-soft` 颜色 token
- `src/components/ui/Button.tsx` — 新增 `variant="brand"` + focus-ring
- `src/components/ui/Input.tsx` — 新增 `variant="box"`
- `src/components/ui/Modal.tsx` — slide-up 动画 + 标题分隔线改 subtle
- `src/components/Sidebar/NavItem.tsx` — 激活态 brand-soft 背景 + brand 色图标 + 左侧 brand 色指示条
- `src/stores/chatStore.ts` — 新增 pinnedSessionIds + recentPrompts 持久化
- `src/stores/uiStore.ts` — 新增 sidebarPinned 状态
- `src/views/ChatView/index.tsx` — 删 4 条横幅，改用 ChatStatusBar 单一组件
- `src/views/ChatView/ChatHeader.tsx` — 用 StatusDot/Spinner 替代 Wifi 图标；模型按钮加 border + bg-surface/60
- `src/views/ChatView/ConversationList.tsx` — 搜索框 + 时间分组 + 右键菜单 + 置顶
- `src/views/ChatView/MessageBubble.tsx` — 头像列布局 + border-left 风格 + StepBlock 折叠 + ReasoningBlock 隐式化
- `src/views/ChatView/MessageInput.tsx` — + 菜单 + @模型 chip + brand ring
- `src/views/ChatView/PermissionDialog.tsx` — 品牌色描边 + 权限图标 + Button 组件
- `src/views/ChatView/ToolCallCard.tsx` — 单行折叠设计 + StatusDot/Spinner 状态
- `src/views/SettingsView/index.tsx` — 两栏布局 + Provider 折叠卡片
- `src/views/SkillsView/index.tsx` — 三栏布局（分类+列表+详情面板）
- `src/views/MemoryView/index.tsx` — Markdown 大纲 + 底部状态栏 + Ctrl+S
- `src/views/McpView/index.tsx` — StatusDot + EmptyHint + 品牌色按钮
- `src/views/TerminalView/index.tsx` — TitleBar + subBar 工具条 + EmptyHint
- `src/App.tsx` — Sidebar 展开状态管理

**设计约定（新代码必须遵守）：**
- 禁止写死颜色（`#fff` / `bg-zinc-800`），一律用 `bg-mc-*` / `text-mc-*` / `border-mc-*` 类名
- 禁止 `text-[Npx]` 任意值，使用标准字号类；如需新字号先在 tailwind config 扩展
- 优先复用原子组件：Spinner / StatusDot / EmptyHint / TitleBar / Button / Input / Modal
- 圆角规范：Button `rounded-md`(6px) / Input卡片 `rounded-lg`(8px) / Modal `rounded-xl`(12px) / Chip `rounded`(4px) / Avatar徽标 `rounded-full`
- 间距用 tailwind 标准刻度 `gap-1/2/3/4` / `px-3/4` / `py-1.5/2/3`，避免任意值 `gap-[5px]`
- 不可变更新原则：SSE handler 和 zustand updater 必须返回新对象/数组
- 5 套主题全部必须能看，改动颜色需同步修改 globals.css 中所有 selector
...
**Windows — NSIS 安装包**
下载 `MiMo-Studio-Setup-1.2.0-win-x64.exe`，双击安装到 Program Files（支持自定义目录）。

**Windows — 免安装版**
下载 `MiMo-Studio-1.2.0-win-x64-Portable.tar.xz`，解压到任意位置：
```bash
...
**Linux — tar.xz**
下载 `MiMo-Studio-1.2.0-linux-x64.tar.xz`，解压即用：
```bash
...
下载对应架构的 tar.xz：
- Apple Silicon (M1/M2/M3/M4)：`MiMo-Studio-1.2.0-mac-arm64.tar.xz`
- Intel：`MiMo-Studio-1.2.0-mac-x64.tar.xz`

```bash
tar xf MiMo-Studio-1.2.0-mac-arm64.tar.xz
