...
| 制品 | 仅 NSIS 安装包 | NSIS 安装包 + macOS DMG (x64/arm64) + Linux deb + 全平台 tar.xz |
...
| 构建目标 | Win nsis / Mac dmg / Linux AppImage+deb | 全平台增加 tar.xz（macOS / Linux）；Windows 仅 NSIS exe |
...
**Windows — NSIS 安装包**
下载 `MiMo-Studio-Setup-<version>-win-x64.exe`，双击安装到 Program Files（支持自定义目录）。

**Linux — tar.xz**
下载 `MiMo-Studio-<version>-linux-x64.tar.xz`，解压即用：
```bash
tar xf MiMo-Studio-<version>-linux-x64.tar.xz
./mimo-studio
```

**Linux — deb**
下载 `MiMo-Studio-<version>-linux-amd64.deb`：
```bash
sudo dpkg -i MiMo-Studio-<version>-linux-amd64.deb
```

**macOS — tar.xz**（推荐，码云可下载）
下载对应架构的 tar.xz：
- Apple Silicon (M1/M2/M3/M4)：`MiMo-Studio-<version>-mac-arm64.tar.xz`
- Intel：`MiMo-Studio-<version>-mac-x64.tar.xz`

```bash
tar xf MiMo-Studio-<version>-mac-arm64.tar.xz
